"""Meta-fiches : picker de fiche parente + garde d'acces.
...
- Une source ne peut pointer vers une fiche parente que si cette fiche est
  dans cet ensemble : sinon un id devine permettrait de rattacher une source
  a une fiche privee d'autrui, et d'en confirmer l'existence.
"""
...
async def cards(db_session, test_user, other_user):
    ...
    made = {
        ...
        "own_private": _card(
            test_user.id, "mon-prive", "Mon prive", visibility="private"
        ),
        ...
        "other_private": _card(
            other_user.id, "leur-prive", "Leur prive", visibility="private"
        ),
    }
...
async def _create_source(client, card_id, parent_card_id):
...
@pytest.mark.asyncio
async def test_source_accepts_own_draft_as_parent(client, session_token, cards):
    client.cookies.set("filum_session", session_token)
    host = cards["own_private"]
    resp = await _create_source(client, host.id, cards["own_draft"].id)
    assert resp.status_code == 201
    assert resp.json()["parent_card_id"] == str(cards["own_draft"].id)


@pytest.mark.asyncio
async def test_source_accepts_other_public_card_as_parent(client, session_token, cards):
    client.cookies.set("filum_session", session_token)
...
@pytest.mark.asyncio
async def test_source_rejects_other_private_card_as_parent(client, session_token, cards):
    ...
    assert resp.json()["error"]["code"] == "invalid_parent_card"


@pytest.mark.asyncio
async def test_source_rejects_other_draft_card_as_parent(client, session_token, cards):
    client.cookies.set("filum_session", session_token)
...
@pytest.mark.asyncio
async def test_source_rejects_unknown_card_as_parent(client, session_token, cards):
    client.cookies.set("filum_session", session_token)
...
@pytest.mark.asyncio
async def test_update_validates_parent_card(client, session_token, cards):
    client.cookies.set("filum_session", session_token)
    ...
    resp = await client.patch(
        ...
        json={"parent_card_id": str(cards["other_private"].id)},
    )
    assert resp.status_code == 400

    resp = await client.patch(
        f"/api/v1/sources/{source_id}",
        json={"parent_card_id": str(cards["other_public"].id)},
    )
    assert resp.status_code == 200
    assert resp.json()["parent_card_id"] == str(cards["other_public"].id)
