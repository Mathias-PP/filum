<script lang="ts">
  ...
  const spiralPath = 'M ' + _spiralPts.map((p) => `${p.x.toFixed(2)} ${p.y.toFixed(2)}`).join(' L ');
  ...
  const currentLogo = {
    branches: [
      { mid: { x: 12, y: 5.5 }, leaves: [{ x: 9, y: 2 }, { x: 15, y: 2 }] },
      { mid: { x: 17.5, y: 9 }, leaves: [{ x: 18.5, y: 4.5 }, { x: 21.5, y: 9 }] },
      { mid: { x: 17.5, y: 15 }, leaves: [{ x: 18.5, y: 19.5 }, { x: 21.5, y: 15 }] },
      { mid: { x: 12, y: 18.5 }, leaves: [{ x: 9, y: 22 }, { x: 15, y: 22 }] },
      { mid: { x: 6.5, y: 15 }, leaves: [{ x: 5.5, y: 19.5 }, { x: 2.5, y: 15 }] },
      { mid: { x: 6.5, y: 9 }, leaves: [{ x: 5.5, y: 4.5 }, { x: 2.5, y: 9 }] },
    ],
  };
</script>
...
<div class="page">
  ...
  <div class="grid">
    ...
    <div class="card reference">
      <div class="canvas">
        <svg viewBox="0 0 24 24" width={scale} height={scale} fill="none" stroke={strokeColor}
    ...
    <div class="card">
      ...
      <p class="caption">Cousin parfaitement symétrique de V8. Racine en bas, 4 feuilles alignées. Versions binaire propre.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">V3 horizontalement, mais avec branches de profondeurs inégales (V8 idée). Suggère que certaines sources ont une chaîne de citations plus longue.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">Arbre phylogénétique classique. Racine à gauche, ramifications par angles droits, feuilles alignées à droite.</p>
    </div>
    ...
    <div class="card">
      <div class="canvas">
        <svg viewBox="0 0 24 24" width={scale} height={scale} fill="none" stroke={strokeColor}
          ...
          {#each [-90, -30, 30, 90, 150, 210] as deg, i (i)}
            ...
            {@const f1 = { x: mid.x + 4.5 * Math.cos(rad) - 1.6 * perp.x, y: mid.y + 4.5 * Math.sin(rad) - 1.6 * perp.y }}
            {@const f2 = { x: mid.x + 4.5 * Math.cos(rad) + 1.6 * perp.x, y: mid.y + 4.5 * Math.sin(rad) + 1.6 * perp.y }}
            <!-- Branche radiale droite -->
            <line x1="12" y1="12" x2={mid.x} y2={mid.y} />
            <!-- Trait latéral perpendiculaire (style cladogramme rectangulaire) -->
            <line x1={mid.x - 1.6 * perp.x} y1={mid.y - 1.6 * perp.y} x2={mid.x + 1.6 * perp.x} y2={mid.y + 1.6 * perp.y} />
            <!-- Deux feuilles -->
      ...
      <p class="caption">V3 disposé radialement (V11 layout). 6 directions, chacune avec un mini-cladogramme rectangulaire. Le motif phylogénétique se répète tout autour.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">6 branches à angles libres : 3 forks (2 feuilles) + 3 simples (1 feuille). Asymétrie qui rappelle un vrai graphe de citations.</p>
    </div>
        ...
        <svg viewBox="0 0 24 24" width={scale} height={scale} fill="none" stroke={strokeColor}
          ...
          {#each [0, 90, 180, 270] as deg, i (i)}
            ...
            {#if isFork}
              ...
              {@const leaf1 = { x: 12 + leafR * Math.cos(rad - spread), y: 12 + leafR * Math.sin(rad - spread) }}
              {@const leaf2 = { x: 12 + leafR * Math.cos(rad + spread), y: 12 + leafR * Math.sin(rad + spread) }}
            ...
            {:else}
              <circle cx={mid.x} cy={mid.y} r={leafRadius * 1.15} fill="currentColor" stroke="none" />
            {/if}
          {/each}
        </svg>
      </div>
      <h3>V6 — Fork+simple à 4 axes</h3>
      <p class="caption">Version minimaliste de V11 : 2 forks (horizontaux) + 2 simples (verticaux). Symétrie d'ordre 2. Plus aéré, plus moderne.</p>
    </div>
    ...
    <div class="card">
      <div class="canvas">
        <svg viewBox="0 0 24 24" width={scale} height={scale} fill="none" stroke={strokeColor}
          ...
          {#each [
            ...
            {@const sub1 = { x: 12 + (leafR + 3) * Math.cos(rad - spread - subSpread), y: 12 + (leafR + 3) * Math.sin(rad - spread - subSpread) }}
            <line x1="12" y1="12" x2={b.kind === 'simple' ? simpleEnd.x : mid.x} y2={b.kind === 'simple' ? simpleEnd.y : mid.y} />
            {#if b.kind === 'simple'}
              <circle cx={simpleEnd.x} cy={simpleEnd.y} r={leafRadius * 1.1} fill="currentColor" stroke="none" />
            {:else}
              <line x1={mid.x} y1={mid.y} x2={leaf1.x} y2={leaf1.y} />
              <line x1={mid.x} y1={mid.y} x2={leaf2.x} y2={leaf2.y} />
              <circle cx={leaf1.x} cy={leaf1.y} r={leafRadius} fill="currentColor" stroke="none" />
              ...
              {#if b.kind === 'fork-deep'}
                ...
                <circle cx={sub1.x} cy={sub1.y} r={leafRadius * 0.75} fill="currentColor" stroke="none" />
              {/if}
            {/if}
          {/each}
        </svg>
      ...
      <p class="caption">Mix de V5 (forks/simples) + V8 (profondeurs variables) en disposition radiale. 2 forks profonds (sous-feuille), 1 fork simple, 2 simples directs.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">Phylogénie verticale, racine en bas. Branches à profondeurs irrégulières — certaines sources sont citées par chaîne plus longue.</p>
    </div>
      ...
      <div class="canvas">
        <svg viewBox="0 0 24 24" width={scale} height={scale} fill="none" stroke={strokeColor}
          ...
          {#each [-90, 30, 150] as deg, i (i)}
            ...
            {@const leaf1 = { x: cross1.x + 2.5 * Math.cos(rad), y: cross1.y + 2.5 * Math.sin(rad) }}
            {@const leaf2 = { x: cross2.x + 2.5 * Math.cos(rad), y: cross2.y + 2.5 * Math.sin(rad) }}
            ...
            <circle cx={leaf1.x} cy={leaf1.y} r={leafRadius * 0.85} fill="currentColor" stroke="none" />
            <circle cx={leaf2.x} cy={leaf2.y} r={leafRadius * 0.85} fill="currentColor" stroke="none" />
          {/each}
        </svg>
      </div>
      <h3>V9 — Triskèle phylogénétique</h3>
      <p class="caption">3 branches à 120° (symétrie de V11), chacune un mini-cladogramme rectangulaire avec 3 feuilles. Croise la pureté géométrique du triskèle avec la rigueur du cladogramme.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">3 branches forkées (2 feuilles) et 3 branches simples (1 feuille) alternées tous les 60° autour du centre. Symétrie d'ordre 3 parfaite. Évolution symétrique de la version précédente.</p>
    </div>
    ...
    <div class="card">
      ...
      <p class="caption">V3 simplifié à l'os : arbre binaire parfait à 2 niveaux, 4 feuilles symétriques. Cadre serré, idéal pour bandeau ou favicon.</p>
    </div>
  </div>
...
<style>
  ...
  :global(.dark) .card.reference {
    background: linear-gradient(180deg, rgba(107, 138, 255, 0.10) 0%, #14141a 100%);
    border-color: rgba(107, 138, 255, 0.30);
  }
  .canvas {
    width: 100%;
    aspect-ratio: 1;
