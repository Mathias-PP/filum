...
- [x] Lot 1 fondations → `01-fondations/` — **Porte G1 VERTE** le 2026-08-25 : 17 fichiers / ~2 540 LOC lus intégralement (sha256 courants vérifiés), 81 symboles documentés avec ancres, spot-check seedé 20260825 (6 items, 6 OK, 1 ancre affinée). Deux bugs du vérificateur découverts et corrigés à cette occasion (`spot_check.sh` pollution du tirage par le seed ; `check_lot.sh` faux positif CONTRADICTION + faux négatif cases vides) → G0 rejouée verte, smoke-tests A/B/C validés, preuve : `_core/preuves/AMENDEMENT_VERIFICATEURS_2026-08-25.md`
- [x] Lot 2 noyau → `02-noyau/` — **Porte G2 VERTE** le 2026-08-25 : `agent.py` (1074 l., 2 tranches), `agent_approvals.py`, `agent_sessions.py` lus intégralement, 41 symboles documentés ; cycle de vie d'un tour écrit événement par événement (9 SSE émis depuis la boucle), compaction 96k/6k + rejeu unique, borne 48 tours + `continuation` documentées. Spot-check seedé 20260825 : 6 items, 6 OK. Dettes relevées : MAX_TOURS figé au niveau module, message `continuation` affichant toujours MAX_TOURS même avec quota_tours différent, commentaire « migration 043 » imprécis (045 en réalité)
- [x] Lot 3 outils MCP → `03-outils-mcp/` — **Porte G3 VERTE** le 2026-08-25 : `server.py` (758 l.), `auth.py` (102 l.), `schema_compat.py` (39 l.), `tools.py` (258 l.), `tools_write.py` (1876 l., 2 tranches) lus intégralement, 55 symboles documentés. Catalogue exhaustif de 45 outils MCP (43 invariant gelé + 2 graphe STARTER non exposés par défaut). Spot-check seedé 20260825 : 6 items, 6 OK. Dérive notée : invariant G0=43 vs code=45 (invariant nb d'outils)
- [x] Lot 4 API → `04-api/` — **Porte G4 VERTE** le 2026-08-25 : 7 endpoints agent (chat SSE, sessions, providers, mode gratuit, définitions, fiche, workspace) lus intégralement, 43 symboles, 31 routes documentées. Spot-check seedé 20260825 : 6 items, 6 OK. Patterns notés : `default=str` comme fusible SSE, `task.cancel()` pour couper la boucle si client déconnecté, `_persister_tour` post-run (pas concurrent), providers ne rendent jamais la clé en clair (invariants endpoints + SSE)
