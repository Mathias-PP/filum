...
`_resolve_base_url` applique ce garde à _toute_ base_url fournie ⇒ **Ollama
...
**Copie du pattern openclaw** : opt-in explicite et _ciblé_ — un flag par provider
...
_joignable ni en loopback ni en RFC1918 depuis la VM_, opt-in ou pas. A1 sert donc aux
...
| **Rotation de clés / credential pools**                 | **Ni interdit ni complexe — différé sur signal.** Complexité réelle mais modérée (S-M) : N clés chiffrées par provider (table fille ou JSONB), rotation **uniquement sur 429** (règle openclaw : tout autre échec est immédiat), affichage masqué de N clés. La valeur est prouvée chez les grands clients : Cherry Studio embarque « multiple-key rotation » officiellement (« avoid rate-limit issues »), openclaw normalise `<PROVIDER>_API_KEYS`. Elle se matérialise surtout sur les **tiers gratuits très limités** (Groq/Gemini) — donc pertinente _après_ A8/A11 qui vont concentrer ce trafic. Déclencheur recommandé : fréquence des 429 constatée en prod > seuil, ou demande utilisateur.   |
...
_Document mis à jour le 2026-08-21 après arbitrage produit ; écrits en lecture seule du dépôt (aucun commit — PR #512 sous le contrôle d'un autre agent)._
