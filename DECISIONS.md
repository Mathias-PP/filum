...
- Coût mesuré (au prototype `/sandbox/hero`) : +0 KB sur le bundle initial des autres routes, +12 KB sur le chunk de la home (lazy), 1-3 % CPU desktop quand visible.
- À implémenter en prod : refactoring du proto sandbox en composant `<HeroPulsar />` avec props (palette, vitesse), fallback SVG inline, lazy import. Détails dans `STATE.md` au paragraphe correspondant.

**Pitfalls rencontrés pendant le proto (à connaître pour future maintenance)**
