...
- **Backend** : tables `agent_lanes` / `agent_lane_usage` / `agent_gratuit_consents` (**migration `051_mode_gratuit`** après renumérotation au merge, seed lane zai = `glm-4.7-flash` (offre gratuite permanente de Z.ai, le flagship glm-5.2 est payant) sur `https://api.z.ai/api/paas/v4`). Les clés ne vivent jamais en base : résolues depuis les settings `agent_gratuit_<slug>_api_key`. Service `agent_gratuit.py` (consentement versionné `2026-08-23-v1`, sélection de lane, cooldown 10 min sur rate-limit, quotas lane + utilisateur), endpoints GET/PUT/DELETE `/agent/mode-gratuit`, intégration dans le chat SSE (ordre de résolution : provider explicite > gratuit > défaut > découverte) avec événement dédié `gratuit_actif`.
...
- **Inertie par défaut** : sans `AGENT_GRATUIT_ENABLED=true` + `AGENT_GRATUIT_ZAI_API_KEY=...` en env, tout est inactif (`disponible=false`, aucun appel sortant).
- **Déployé et activé en prod le 2026-08-23** (PR #542 + #541-#547 embarquées). Vérifié sur la VM : `.env` complété (`agent_gratuit_enabled=true`, clé Z.ai posée), conteneur recréé (`--force-recreate backend` pour charger l'env modifié), `alembic_version = 051_mode_gratuit`, `agent_lanes = 1`, démarrage sans erreur, `/health` OK depuis l'extérieur, route `/api/v1/agent/mode-gratuit` répond 401 sans session. Test navigateur (consentement puis message streamé) à confirmer par l'utilisateur.
