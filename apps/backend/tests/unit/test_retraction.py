...
class TestSchedule:
    """Le garde-fou anti-doublon : une fiche publique populaire est servie des
    dizaines de fois par minute, et relancerait la meme verification a chaque
    requete tant que la premiere n'a pas commit."""

    def test_une_source_deja_en_vol_n_est_pas_replanifiee(self, monkeypatch):
        from uuid import uuid4

        from app.services import retraction_check as rc

        spawned: list[list] = []
        monkeypatch.setattr(rc.asyncio, "create_task", lambda coro: (coro.close(), _Noop())[1])
        monkeypatch.setattr(rc, "_run", lambda pairs: spawned.append(pairs) or _coro())

        sid = uuid4()
        rc.schedule_retraction_checks([(sid, "10.1/x")])
        rc.schedule_retraction_checks([(sid, "10.1/x")])

        assert len(spawned) == 1
        rc._in_flight.discard(sid)

    def test_liste_vide_ne_planifie_rien(self, monkeypatch):
        from app.services import retraction_check as rc

        called = []
        monkeypatch.setattr(rc.asyncio, "create_task", lambda coro: called.append(1))
        rc.schedule_retraction_checks([])
        assert called == []
