...
- ~~**Ancrage d'extrait fuzzy (§3.5)**~~ ✅ #333. Un extrait ne portait que son texte : la source corrigeait une coquille et le passage devenait introuvable — donc lisible comme une citation inventée. Désormais on persiste le voisinage (48 caractères de part et d'autre) et la position, et `POST /sources/{id}/excerpts/verify` ré-ancre dans la page telle qu'elle est aujourd'hui. Deux points de doctrine, tenus par les tests : **un ancrage complaisant est pire que pas d'ancrage** (en dessous du seuil de ressemblance on ne rend rien, plutôt que de désigner un passage plausible mais faux), et **`unreadable` n'est pas `missing`** — une page qui ne rend aucun texte ne dit rien sur l'extrait ; les confondre ferait passer une source inaccessible pour une citation fabriquée. Reste à faire : aucun écran n'appelle encore `/verify`.
