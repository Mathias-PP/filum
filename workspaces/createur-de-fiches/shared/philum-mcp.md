...
| `propose_passages(url, questions)` | 02 | Rend, pour chaque question, les passages exacts de la page les plus proches par le sens. À appeler sur une adresse candidate avant `add_source`. Ne pose rien. |
| `add_source(card_slug, metadata_from, url?, title?, authors?, doi?, category?, author_kind?, format?, stance?, annotation?, journal?, archive_url?, excerpts?)` | 02 puis 03 | Une IA pose la source avec ses extraits (`excerpts`, passages retenus de `propose_passages`) : sans extrait retrouve dans la page, rien n'est écrit. Seul un lien vers une autre fiche Philum passe sans extrait. |
...
| `add_sources_batch(card_slug, sources)` | 02 | Ajoute plusieurs sources en un appel. Chaque entrée porte ses `excerpts` : une entrée sans extrait retrouvé part dans `failed`. |
...
- **Ordre d'appel** : `create_card` → pour chaque candidate `propose_passages` puis `add_source` avec ses `excerpts` → `add_excerpt` pour un passage de plus → `set_content_text` (optionnel) → connexions (étape 05) → `publish_card`.
