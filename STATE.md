...
_Aucune._ Session 2026-07-30 (autonome) — **refonte du pipeline d'extraction v2, agnostique** :
...
- **Méta-fiches** (PR #214) : `sources.parent_card_id` (migration `015`) permet de rattacher une source à une **fiche entière**, en pendant de `parent_source_id`. À ne pas confondre avec `linked_card_id` (« cette source EST cette fiche », résolu depuis l'URL). `GET /cards/search` sert le picker (fiches de l'user, brouillons compris, + toutes les fiches publiées et publiques) et `assert_parent_card_allowed` revalide ce périmètre à l'écriture, sinon un id deviné permettrait de confirmer l'existence d'une fiche privée d'autrui. UI : sources numérotées + filtre, et une seconde liste déroulante avec sa propre recherche pour les fiches.
...
- **Rendu public des méta-fiches** — `parent_card_id` est saisissable et stocké, mais la fiche publiée n'affiche pas encore le lien vers la fiche parente (le dashboard se contente d'un badge « rattachée à une fiche »). Reste à enrichir `SourceResponse` avec titre/slug/créateur de la fiche parente et à l'exposer côté public.
