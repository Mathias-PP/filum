...
- `datetime.utcnow()` est déprécié en Python 3.12. **Toujours** utiliser `datetime.now(UTC).replace(tzinfo=None)` pour matcher les colonnes `DateTime` sans `timezone=True`. Cas vécu (PRs #33-#36, mai 2026) : oubli du `.replace(tzinfo=None)` dans `publish_card` → `asyncpg.DataError` → session corrompue → `get_db` cleanup re-commit → réponse interrompue mid-stream → user voit `Failed to fetch` + erreur CORS trompeuse. Chassé pendant 4 PRs avant que l'endpoint `/health/publish-diagnose` n'expose la vraie cause.
- **Debug bug prod résistant** : si un bug résiste à 1 fix, **stop spéculer, instrumenter**. Endpoint `/health/<feature>-diagnose` qui exerce le code path sans auth + retourne traceback. Voir `agent/PITFALLS.md` §1.5 cas vécu.
